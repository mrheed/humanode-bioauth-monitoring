...
const sendExpiryWarning = (difference) => {
  if (difference > (60 * 5.5) && difference < ((60 * 6) - 1)) {
    sendTelegramMessage(`🟢Your Humanode ${config.validatorName} will be deactivated in 6 hours, please prepare for re-authentication ${config.telegramUserTag} in <a href="${getState('wsTunnelURL')}">here</a>`);
  } else if (difference > 25 && difference < 31) {
    ...
    let message = `🟢Your Humanode ${config.validatorName} still have ${parseInt(difference / 60)} hours ${parseInt(difference % 60)} munites left to be deactivated ${config.telegramUserTag}`
    if (config.alwaysSendToTelegram) {
