...
const sendExpiryWarning = (difference) => {
  if (difference > 25 && difference < 31) {
    sendTelegramMessage(`🟡Your Humanode ${config.validatorName} will be deactivated in 30 minutes, please prepare for re-authentication ${config.telegramUserTag}`);
  } else if (difference > 0 && difference < 6) {
    sendTelegramMessage(`Your Humanode ${config.validatorName} will be deactivated in 5 minutes, please prepare for re-authentication ${config.telegramUserTag}`);
  } else {
    sendTelegramMessage(`You Humanode ${config.validatorName} still have ${parseInt(difference / 60)} hours ${parseInt(difference / 60) % 60} munites left to be deactivated`)
  }
};
...
export const checkHumanodeStatus = async () => {
  try {
    ...
    if (status === 'inactive') {
      sendTelegramMessage(`Your Humanode ${config.validatorName} is not active, please proceed to do re-authentication ${config.telegramUserTag}`);
